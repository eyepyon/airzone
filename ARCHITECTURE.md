...
External Services:
┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐
│  Sui Blockchain  │  │  Stripe API      │  │  Google OAuth    │
...
3. **ユーザー体験:** ユーザーは SUI トークンを持たずに NFT を受け取れる

**フロー:**
```
1. ユーザーが NFT 発行をリクエスト
2. Backend がスポンサーウォレットでトランザクションに署名
3. Sui ネットワークにトランザクション送信
4. スポンサーがガス代を支払う
...
- **スポンサードトランザクション:** ユーザーはガス代不要で NFT を受け取れる
- **自動ウォレット生成:** ユーザー登録時に Sui ウォレットを自動作成
