...
<h1 class="text-3xl font-bold mb-6">エスクローキャンペーン編集</h1>

<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.escrow-campaigns.update', $campaign->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" rows="3"
                      class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500">{{ old('description', $campaign->description) }}</textarea>
        </div>

        <div class="grid grid-cols-3 gap-4 mb-4">
            <div>
                <label class="block text-gray-700 font-bold mb-2">最小金額 (XRP) <span class="text-red-500">*</span></label>
                <input type="number" id="min_amount_xrp" step="0.01" min="0"
                       value="{{ old('min_amount_xrp', $campaign->min_amount_drops / 1000000) }}" required
                       class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500"
                ...
                <input type="hidden" id="min_amount_drops" name="min_amount_drops"
                       value="{{ old('min_amount_drops', $campaign->min_amount_drops) }}">
            </div>

            <div>
                <label class="block text-gray-700 font-bold mb-2">ロック期間 (日) <span class="text-red-500">*</span></label>
                <input type="number" name="lock_days" value="{{ old('lock_days', $campaign->lock_days) }}" min="1" required
                       class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500">
            </div>

            <div>
                <label class="block text-gray-700 font-bold mb-2">最大参加者数</label>
                <input type="number" name="max_participants" value="{{ old('max_participants', $campaign->max_participants) }}" min="1"
                       class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nft_reward_name" value="{{ old('nft_reward_name', $campaign->nft_reward_name) }}" required
                   class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500">
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="nft_reward_description" rows="3"
                      class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500">{{ old('nft_reward_description', $campaign->nft_reward_description) }}</textarea>
        </div>

        <div class="mb-6">
            <label class="block text-gray-700 font-bold mb-2">NFT画像URL</label>
            <input type="url" name="nft_reward_image_url" value="{{ old('nft_reward_image_url', $campaign->nft_reward_image_url) }}"
                   class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500">
        </div>
